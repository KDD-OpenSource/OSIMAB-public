...
from tfsnippet.utils import get_default_session_or_error, ensure_variables_initialized
...
class QuietDonutTrainer(DonutTrainer):
    def fit(
        ...
        if labels.shape != values.shape:
            raise ValueError(
                "The shape of `labels` does not agree with "
                "the shape of `values` ({} vs {})".format(labels.shape, values.shape)
            )
        if missing.shape != values.shape:
            raise ValueError(
                "The shape of `missing` does not agree with "
                "the shape of `values` ({} vs {})".format(missing.shape, values.shape)
            )
...
class Donut(Algorithm, TensorflowUtils):
    ...
    def fit(self, X: pd.DataFrame):
        with self.device:
            ...
            for col_idx in trange(len(X.columns)):
                ...
                timestamps, _, (features, labels) = complete_timestamp(
                    timestamps, (features, labels)
                )
                missing = np.isnan(X.loc[:, col].values)
                _, mean, std = standardize_kpi(
                    features, excludes=np.logical_or(labels, missing)
                )
                ...
                with tf_session.as_default():
                    trainer.fit(
                        features, labels, missing, mean, std, valid_portion=0.25
                    )
    ...
    def predict(self, X: pd.DataFrame):
        ...
        with self.device:
            ...
            aggregated_test_scores[: self.x_dims - 1] = (
                np.nanmin(aggregated_test_scores) - sys.float_info.epsilon
            )
