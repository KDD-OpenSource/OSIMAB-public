...
class AutoCorrelationEncoder(Algorithm, PyTorchUtils):
    def __init__(
        self,
        name: str = "AutoCorrelationEncoder",
        num_epochs: int = 10,
        batch_size: int = 20,
        lr: float = 1e-3,
        hidden_size: int = 5,
    ...
    def fit(self, X):
        ...
        split_point = len(sequences) - int(
            self.train_gaussian_percentage * len(sequences)
        )
        ...
        self.ae_list = [
            AutoEncoderModule(
                1, self.sequence_length, self.hidden_size, seed=self.seed, gpu=self.gpu
            )
        ...
        if self.use_threading:
            ...
            for channel, ae in enumerate(self.ae_list):
                ...
                thread = threading.Thread(
                    target=train_ae, args=(ae, seq_channel, channel, self.lr)
                )
                thread.start()
        ...
        for ts_batch in train_gaussian_loader:
            output = [
                ae(self.to_var(ts_batch[:, :, channel]))
                for channel, ae in enumerate(self.ae_list)
            ]
    ...
    def generate_enc(self, data_loader):
        ...
        for ts_batch in data_loader:
            enc = [
                ae(self.to_var(ts_batch[:, :, channel]), return_latent=True)[1]
                for channel, ae in enumerate(self.ae_list)
            ]
