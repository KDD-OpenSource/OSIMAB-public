...
class LSTMAD(Algorithm, PyTorchUtils):
    ...
    def fit(self, X):
        ...
        self.model.eval()
        input_data_gaussian, target_data_gaussian = self._input_and_target_data_eval(
            X_train_gaussian
        )
        ...
        norm = errors.reshape(
            errors.shape[0] * errors.shape[1], X.shape[-1] * self.len_out
        )
    ...
    def predict(self, X):
        ...
        errors, stacked_preds = self._calc_errors(
            predictions, target_data, return_stacked_predictions=True
        )

        if self.details:
            self.prediction_details.update(
                {
                    "predictions_mean": np.pad(
                        stacked_preds.mean(axis=3).squeeze(0).T,
                        ((0, 0), (self.len_in + self.len_out - 1, 0)),
                        "constant",
            ...
            self.prediction_details.update(
                {
                    "errors_mean": np.pad(
                        errors.mean(axis=3).reshape(-1),
                        (self.len_in + self.len_out - 1, 0),
                        "constant",
                        constant_values=np.nan,
                    )
                }
        ...
        scores = -multivariate_normal.logpdf(
            norm, mean=self.mean, cov=self.cov, allow_singular=True
        )
        scores = np.pad(
            scores,
            (self.len_in + self.len_out - 1, 0),
    ...
    def _input_and_target_data(self, X: pd.DataFrame):
        ...
        input_data = self.to_var(
            torch.from_numpy(X[:, : -self.len_out, :]), requires_grad=False
        )
        target_data = []
        for l in range(self.len_out - 1):
            target_data += [X[:, 1 + l : -self.len_out + 1 + l, :]]
        target_data += [X[:, self.len_out :, :]]
        target_data = self.to_var(
            torch.from_numpy(np.stack(target_data, axis=3)), requires_grad=False
        )
    ...
    def _input_and_target_data_eval(self, X: pd.DataFrame):
        ...
        target_data = self.to_var(
            torch.from_numpy(X[:, self.len_in + self.len_out - 1 :, :]),
            requires_grad=False,
        )
        return input_data, target_data
    ...
    def _calc_errors(self, predictions, target_data, return_stacked_predictions=False):
        ...
        for l in range(1, self.len_out):
            errors += [
                predictions.data.numpy()[
                    :, self.len_out - 1 - l : -self.len_in - l, :, l
                ]
            ]
        ...
        return (
            errors
            if return_stacked_predictions is False
            else (errors, stacked_predictions)
        )

    def _build_model(self, d, batch_size):
        self.model = LSTMSequence(
            d, batch_size, len_in=self.len_in, len_out=self.len_out
        )
...
class LSTMSequence(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        outputs = torch.stack(
            outputs, 1
        ).squeeze()  # stack (n, d * len_out) outputs in time dimensionality (dim=1)
