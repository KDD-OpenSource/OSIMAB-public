...
class RecurrentEBM(Algorithm, TensorflowUtils):
    ...
    def predict(self, X):
        ...
        with self.device:
            ...
            for i in range(len(X)):
                reconstruction_err = self.tf_session.run(
                    [self.cost],
                    feed_dict={self.input_data: X[i : i + 1], self._batch_size: 1},
                )
    ...
    def _build_model(self, n_visible):
        self.input_data, self.lr, self._batch_size = self._create_placeholders(
            n_visible
        )
        (
            self.W,
            ...
            self.BH_t,
            self.BX_t,
        ) = self._create_variables(n_visible)
        ...
        def rnn_recurrence(u_tmin1, sl):
            ...
            u_t = tf.nn.softplus(
                self.bu + tf.matmul(sl, self.Wxu) + tf.matmul(u_tmin1, self.Wuu)
            )
            return u_t
        ...
        self.BH_t = tf.reshape(
            tf.scan(
                hidden_bias_recurrence, u_t, tf.zeros([1, self.n_hidden], tf.float32)
            ),
    ...
    def _create_variables(self, n_visible):
        W = tf.Variable(
            tf.random_normal([n_visible, self.n_hidden], stddev=0.01), name="W"
        )
        Wuh = tf.Variable(
            tf.random_normal([self.n_hidden_recurrent, self.n_hidden], stddev=0.01),
            name="Wuh",
        )
        Wux = tf.Variable(
            tf.random_normal([self.n_hidden_recurrent, n_visible], stddev=0.01),
            name="Wux",
        )
        Wxu = tf.Variable(
            tf.random_normal([n_visible, self.n_hidden_recurrent], stddev=0.01),
            name="Wxu",
        )
        Wuu = tf.Variable(
            tf.random_normal(
                [self.n_hidden_recurrent, self.n_hidden_recurrent], stddev=0.01
            ),
            name="Wuu",
        )
