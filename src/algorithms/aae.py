...
class AdversarialAutoEncoder(Algorithm, PyTorchUtils):
    def __init__(self, name: str = 'AdverserialAutoEncoder',
                 num_epochs: int = 10, batch_size: int = 20, lr: float = 1e-3,
                 hidden_size: int = 5, sequence_length: int = 30,
                 train_gaussian_percentage: float = 0.25,
                 seed: int = None, gpu: int = None, details=True, activation='Tanh'):
    ...
    def fit(self, X: pd.DataFrame):
        ...
        self.discriminator = DiscriminatorModule(self.hidden_size, seed=self.seed, gpu=self.gpu,
                                                 activation=self.activation)
        ...
        for epoch in trange(self.num_epochs):
            ...
            for ts_batch in train_loader:
                ...
                loss_d = -torch.mean(torch.log(disc_real + 1e-15) + torch.log(1 - disc_fake + 1e-15))
                loss_d.backward()
                optimizer_d.step()
...
class AutoEncoderModule(nn.Module, PyTorchUtils):
    def __init__(self, n_features: int, sequence_length: int, hidden_size: int, seed: int, gpu: int,
        ...
        layers = np.array([[nn.Linear(int(a), int(b)), activation()] for a, b in enc_setup.reshape(-1, 2)]).flatten()[
                 :-1]
        self._encoder = nn.Sequential(*layers)
        self.to_device(self._encoder)

        layers = np.array([[nn.Linear(int(a), int(b)), activation()] for a, b in dec_setup.reshape(-1, 2)]).flatten()[
                 :-1]
