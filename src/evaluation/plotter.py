...
class Plotter:
    def __init__(
        self, output_dir, pickle_dirs=None, dataset_names=None, detector_names=None
    ):
    ...
    def import_results_for_runs(self, pickle_dirs=["data"]):
        ...
        for dir_ in pickle_dirs:
            for path in os.listdir(os.path.join(dir_, "evaluators")):
                ...
                assert self.dataset_names is None or np.array_equal(
    ...
    def barplots(self, title):
        ...
        fig, axes = plt.subplots(
            ncols=len(self.detector_names),
            figsize=(1.5 * len(self.detector_names), 4),
            sharey=True,
        )
    ...
    def _styled_boxplot(self, ax, aurocs_df, det):
        ...
        ax.boxplot(
            [ds_groups.get_group(x)["auroc"].values for x in self.dataset_names],
            positions=np.linspace(0, 1, len(self.dataset_names)),
            widths=0.15,
        ...
        ax.set_xticklabels(
            [
                f"{float(Evaluator.get_key_and_value(x)[1]):.2f}"
