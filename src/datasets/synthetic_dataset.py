...
class SyntheticDataset(Dataset):
    ...
    def load(self):
        generator = MultivariateDataGenerator(
            self.length,
            self.n,
            self.k,
        ...
        y_train = self._label_outliers(self.label_config or self.pollution_config)[
            :train_split_point
        ]

        X_test = generator.add_outliers(self.outlier_config)[train_split_point:]
        y_test = self._label_outliers(self.label_config or self.outlier_config)[
            train_split_point:
        ]
    ...
    def add_missing_values(self, missing_percentage: float, per_column: bool = True):
        ...
        if per_column:
            for col in X_train.columns:
                missing_idxs = np.random.choice(
                    len(X_train), int(missing_percentage * len(X_train)), replace=False
                )
