...
class SyntheticMultivariateDataset(Dataset):
    def __init__(
        ...
        assert (
            features >= 2
        ), "At least two dimensions are required for generating MV outliers"
        self.features = features
        assert group_size is None or (features >= group_size > 0), (
            "Group size may not be greater " "than amount of dimensions"
        )
        self.group_size = group_size or self.features
        assert self.group_size <= self.features
        if (
            self.features % self.group_size == 1
        ):  # How many dimensions each correlated group has
            logging.warn(
                "Group size results in one overhanging univariate group. Generating multivariate"
    ...
    def get_random_curve(self, length_randomness=10, amplitude_randomness=1):
        ...
        new_amplitude = self.get_noisy_value(
            sign * self.mean_curve_amplitude, amplitude_randomness
        )
    ...
    def generate_correlated_group(self, dimensions, pollution):
        ...
        while pos < self.length - self.mean_curve_length - 20:
            ...
            create_anomaly = np.random.choice(
                [False, True], p=[1 - pollution, pollution]
            )
            # After curve add pause, only noise
            end_of_interval = pos + len(curve) + self.create_pause()
            self.insert_features(
                values[pos:end_of_interval, :],
                labels[pos:end_of_interval],
                curve,
                create_anomaly,
            )
            pos = end_of_interval
    ...
    def generate_data(self, pollution):
        ...
        for i in range(0, self.features, self.group_size):
            values, labels = self.generate_correlated_group(
                min(self.group_size, self.features - i),
    ...
    def insert_features(
        ...
        for i in set(range(interval_values.shape[1])) - {anomaly_dim}:
            ...
            interval_values[len(curve) :, i] = self.add_global_noise(
                interval_values[len(curve) :, i]
            )
        ...
        anomaly_values, start, end = self.anomaly_func(
            curve, create_anomaly, interval_length
        )
        assert (
            len(anomaly_values) <= interval_length
        ), f"Interval too long: {len(anomaly_values)} > {interval_length}"
        ...
        if create_anomaly:
            ...
            ), f"Invalid anomaly indizes: {start} to {end}"
