...
def generate_timestamps(start, end, percentage):
    ...
    timestamps = [
        (w, w + WINDOW_SIZE)
        for w in np.random.choice(
            windows, int(percentage * len(windows)), replace=False
        )
    ]
    return timestamps
...
class LongTermDependencyDataset(Dataset):
    ...
    def __init__(self, name, file_name, random_state, n=100, gaussian_std=0.01):
        ...
        self.height_mask = np.tile([0, 1.0], n)
...
class SyntheticDataGenerator:
    ...
    * see outlier_config
    ...
    def extreme_1(seed, n=1, k=1, anomaly_percentage=0.023):
        ...
        timestamps = [
            (t,)
            for t in np.random.randint(
                0, length - train_size, int(anomaly_percentage * (length - train_size))
            )
            + train_size
        ]
        ...
        return SyntheticDataset(
            name=f"Syn Extreme Outliers (dim={n})",
            file_name="extreme1.pkl",
            length=length,
            n=n,
            k=k,
            ...
            pollution_config=pollution_config,
            train_split=train_split,
            random_state=random_state,
        )

    @staticmethod
    def extreme_1_polluted(
        seed, pollution_percentage=0.2, n=1, anomaly_percentage=0.023
    ):
        """Full pollution -> All anomalies from test set are in train set"""
        np.random.seed(seed)
        dataset = SyntheticDataGenerator.extreme_1(
            seed, n, anomaly_percentage=anomaly_percentage
        )
        ...
        timestamps = [
            (t,)
            for t in np.random.randint(
                0, train_size, int(pollution_percentage * train_size)
            )
        ]
    ...
    def shift_1(seed, n=1, k=1, anomaly_percentage=0.2):
        ...
        timestamps = generate_timestamps(
            int(train_split * length), length, anomaly_percentage
        )
        ...
        return SyntheticDataset(
            name=f"Syn Shift Outliers (dim={n})",
            file_name="shift1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def shift_1_polluted(seed, pollution_percentage=0.2, n=1, anomaly_percentage=0.2):
        ...
        timestamps = generate_timestamps(
            0, int(dataset.train_split * dataset.length), pollution_percentage
        )
    ...
    def variance_1(seed, n=1, k=1, anomaly_percentage=0.2):
        ...
        return SyntheticDataset(
            name=f"Syn Variance Outliers (dim={n})",
            file_name="variance1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    @staticmethod
    def variance_1_polluted(
        seed, pollution_percentage=0.2, n=1, anomaly_percentage=0.2
    ):
        np.random.seed(seed)
    ...
    def trend_1(seed, n=1, k=1, anomaly_percentage=0.2):
        ...
        return SyntheticDataset(
            name=f"Syn Trend Outliers (dim={n})",
            file_name="trend1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def combined_1(seed, n=1, k=1):
            ...
            "extreme": [
                {
                    "n": i,
                    "timestamps": [
                        ts for d, ts in zip(dim_ext, timestamps_ext) if d == i
                    ],
                }
                for i in range(n)
            ],
            "shift": [
                {
                    "n": i,
                    "timestamps": [
                        ts for d, ts in zip(dim_shi, timestamps_shi) if d == i
                    ],
                }
                for i in range(n)
            ],
            "variance": [
                {
                    "n": i,
                    "timestamps": [
                        ts for d, ts in zip(dim_var, timestamps_var) if d == i
                    ],
                }
                for i in range(n)
            ],
            "trend": [
                {
                    "n": i,
                    "timestamps": [
                        ts for d, ts in zip(dim_tre, timestamps_tre) if d == i
                    ],
                }
                for i in range(n)
            ],
        ...
        return SyntheticDataset(
            name=f"Synthetic Combined Outliers (dim={n})",
            file_name="combined1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def mv_extreme_1(seed, n=2, k=2):
        ...
        label_config = {"outlier_at": [{"timestamps": [(2695,)]}]}
        random_state = seed

        return SyntheticDataset(
            name=f"Synthetic Multivariate Extreme Outliers (dim={n})",
            file_name="mv_extreme1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def mv_shift_1(seed, n=2, k=2):
        ...
        outlier_config = {
            "shift": [
                ...
                {"n": 1, "timestamps": [(2210, 2270), (2500, 2580), (2800, 2900)]},
            ]
        }
        pollution_config = {
            "shift": [
        ...
        label_config = {"outlier_at": [{"timestamps": [(2300, 2340), (2600, 2650)]}]}
        random_state = seed

        return SyntheticDataset(
            name=f"Synthetic Multivariate Shift Outliers (dim={n})",
            file_name="mv_shift1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def mv_variance_1(seed, n=2, k=2):
        ...
        label_config = {"outlier_at": [{"timestamps": [(2500, 2550)]}]}
        random_state = seed

        return SyntheticDataset(
            name=f"Synthetic Multivariate Variance Outliers (dim={n})",
    ...
    def mv_trend_1(seed, n=2, k=2):
        ...
        outlier_config = {
            "trend": [
                {"n": 0, "timestamps": [(2200, 2400), (2450, 2480), (2700, 2950)]},
                {
                    "n": 1,
                    "timestamps": [
                        (2200, 2400),
        ...
        pollution_config = {
            "trend": [
                {
                    "n": 0,
                    "timestamps": [(200, 400), (550, 420), (500, 550), (1200, 1350)],
                },
                {
                    "n": 1,
                    "timestamps": [(200, 400), (550, 420), (500, 550), (1200, 1350)],
        ...
        return SyntheticDataset(
            name=f"Synthetic Multivariate Trend Outliers (dim={n})",
            file_name="mv_trend1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def mv_xor_extreme_1(seed, n=2, k=2):
        ...
        return SyntheticDataset(
            name=f"Synthetic Multivariate XOR Extreme Outliers (dim={n})",
            file_name="mv_xor_extreme1.pkl",
            length=length,
            n=n,
            k=k,
            baseline_config=baseline_config,
    ...
    def behavior_sine_1(seed, cycle_length=200, n=1, k=1):
        ...
        return SyntheticDataset(
            name="Synthetic Seasonal Outliers (dim={n})",
            file_name="sine1.pkl",
            length=length,
            n=n,
            k=k,
    ...
    def long_term_dependencies_missing(seed):
        return LongTermDependencyDataset(
            "Long Term Dependencies Missing", "long_term_missing", seed
        ).build_missing_data([153, 155])

    @staticmethod
    def long_term_dependencies_height(seed):
        return LongTermDependencyDataset(
            "Long Term Dependencies Halved Height", "long_term_halved_height", seed
        ).build_halved_height([153, 155])

    @staticmethod
    def long_term_dependencies_width(seed):
        return LongTermDependencyDataset(
            "Long Term Dependencies Irregular Length",
            "long_term_irregular_length",
            seed,
        ).build_irregular_widths([153, 155], [10, 30])
