...
def catman_to_df(files, cfg=None, columns=None, standardize=True):
    """
    ...
    for cm_file in files:
        df = read_catman_file(cm_file, cfg)
...
def read_catman_file(file_name, cfg):
...
def main():
    ...
    sensorData = catman_to_df(files, cfg=cfg)
    sensorDataFiltered = []
