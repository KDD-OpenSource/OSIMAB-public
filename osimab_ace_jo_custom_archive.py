...
def evaluate_osimab_jo():
    ...
    # elif len(sys.argv) == 2:
    #    cfgs.append(config(external_path = os.path.join("./dailyconfig/",
    #        'config_daily.yaml')))
    ...
    for cfg in cfgs:
        ...
        if cfg.dataset.regexp_bin_train is not None:
            ...
            # filenames_train, pathnames_train = remove_files(files_blocklist,
            #        filenames_train, pathnames_train, cfg)
            ...
            #    'tmp','whiteList.txt'), 'r') as whitelist_file:
        ...
        #                'PredictionResults', predictionDate)
        #            output_dirs.append(output_dir)
        #            os.mkdir(output_dir)
        #            test_days.append([OSIMABDataset(cfg, file_name = filename)
        #                for filename in pathnames_test if predictionDate in filename])
        ...
        if cfg.ace.load_file is not None:
            ...
            # check how datasets_train looks like and if the space was freed
        ...
        #        evaluator = Evaluator(test_days[testDayCounter], dets, seed=seed, cfg=cfg,
...
def get_configs():
    ...
    if len(sys.argv) == 1:
        for file_name in os.listdir("./configs"):
            if file_name.endswith(".yaml"):
                cfgs.append(
                    config(external_path=os.path.join("./configs/", file_name))
                )
    elif len(sys.argv) == 2:
        cfgs.append(
            config(external_path=os.path.join("./dailyconfig/", "config_daily.yaml"))
        )
        cfgs[0].config_dict["ctx"] += time.strftime("%Y-%m-%d-%H%M%S")
        cfgs[0].ctx += time.strftime("%Y-%m-%d-%H%M%S")
    else:
...
def predict_testsets(datasets_test, dets, output_dir, seed, cfg):
    evaluator = Evaluator(
        datasets_test, dets, seed=seed, cfg=cfg, output_dir=output_dir
    )
    evaluator.evaluate()
    result = evaluator.benchmarks()
    evaluator.plot_roc_curves()
